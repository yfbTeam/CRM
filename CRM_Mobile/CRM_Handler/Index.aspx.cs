...
namespace CRM_Handler
{
    public partial class Index : System.Web.UI.Page
    {
                    ...
                    if ((listU != null && listU.Count > 0) || userName == pub_cust_name)
                    {
                        if (dic_data != null && dic_data.Count > 0)
                        {
                            if (dic_data[0].Count >= 14)
                            {
                                ...
                                Label1.Text = "成功导入客户数量：" + add_customer_count + "。成功导入联系人数量：" + add_linkman_count + "。已存在客户数量：" + exit_customer_count + "。已存在联系人数量：" + exit_linkman_count;

                            }
                            else
                            {
                                Label1.Text = "excel模板不正确，请联系管理员";
                            }
                        }
                        else
                        {
                            Label1.Text = "未读取到excel数据";
                        }
                    }
                    else
                    {
                        //用户名不存在
                        Label1.Text = "用户不存在或未授权";
                    }
                }
                else
                {
                    //未输入用户名称
                    Label1.Text = "用户名不存在或未授权";
                }
            }
            catch (Exception ex)
            {
        ...
        private long custom_edit(List<Dictionary<string, object>> dic_data, int i, List<cust_customer> customers, long customerID)
        {
            ...
            exit_customer_count++;
            this.Panel1.Controls.Add(new TextBox() { Width = new Unit(100, UnitType.Percentage), Text = i + "、该客户已存在：" + cust_select.cust_name + "      负责人：" + customers[0].cust_usersname + Environment.NewLine, ForeColor = Color.Red });
            this.Panel1.Controls.Add(new Literal() { Text = "<br />" });
            return customerID;
        }
        ...
        private long custom_add(List<Dictionary<string, object>> dic_data, string userName, string uniQ, int i, string customName, long customerID)
        {
            ...
            try
            {
                ...
                {
                    add_customer_count++;
                    this.Panel1.Controls.Add(new TextBox() { Width = new Unit(100, UnitType.Percentage), Text = i + "、成功提交客户：" + customName + "      负责人：" + userName + Environment.NewLine });
                    this.Panel1.Controls.Add(new Literal() { Text = "<br />" });
                }
        ...
        private void linkman_add(List<Dictionary<string, object>> dic_data, string userName, string uniQ, int i, string customName, long customerID, string link_name, string link_phonenumber, DateTime d_1, int link_level, string sex, string position, string email, string telep, string remark)
        {
            try
            {
                ...
                {
                    add_linkman_count++;
                    this.Panel1.Controls.Add(new TextBox() { Width = new Unit(100, UnitType.Percentage), Text = i + "、成功提交联系人：" + link_name + "      负责人：" + userName + Environment.NewLine });
                    this.Panel1.Controls.Add(new Literal() { Text = "<br />" });
        ...
        private void linkman_edit(List<Dictionary<string, object>> dic_data, string userName, int i, string link_name, DateTime d_1, int link_level, string sex, string position, string email, string telep, string remark, List<cust_linkman> customers)
        {
            try
            {
                ...
                exit_linkman_count++;
                this.Panel1.Controls.Add(new TextBox() { Width = new Unit(100, UnitType.Percentage), Text = i + "、联系人已存在【客户名称、联系人名称、手机号相同】,联系人信息覆盖：" + link_name + "      负责人：" + userName + Environment.NewLine, ForeColor = Color.Red });
                this.Panel1.Controls.Add(new Literal() { Text = "<br />" });
            }
            catch (Exception ex)
            {
